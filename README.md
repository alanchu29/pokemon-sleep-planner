...
- **EX 營地（專家模式）有算** — 研究區域選「萌綠之島EX」「天青沙灘EX」即可；主要樹果加速與主技能等級 +1、
  非喜好樹果的幫忙間隔懲罰、以及三種隨機營地效果（樹果 2.4 倍／食材 +1／技能發動率 ×1.25）都納入。
  其中「食材專長有時候額外 +1」的機率遊戲沒公布，沿用上游的 50%；天青沙灘EX 的數值只有上游程式碼當來源。
  寶可夢箱的個體產能與截圖匯入**刻意不套 EX**
- **本週活動加成（自訂）有算** — 「本週條件」底下的「本週活動加成」可以自己勾選並填數值：
  主技能發動率／樹果能量／食材獲得量／料理能量各 +N%，大成功機率 +N **個百分點**（單位不同）。
  和 EX 營地效果獨立相乘；樹果與食材**只算幫忙撿來的那一份**（主技能發出／灑出的不吃，這是刻意的範圍）。
  寶可夢箱的個體產能刻意不套
- **「料理機率提升」系主技能（美味機會S／怪力鉗）的貢獻修正過（2026-09-11）** — 以前少除了「一天 3 餐」，
  淨效果高估約 2.1 倍。沒帶這類技能的隊伍逐位不變
- 週日單獨最佳化、薰香：均未納入
