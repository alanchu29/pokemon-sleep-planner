...
**食材篩選（`fltIng`）是唯一可以跨專長排的名次。** 同一組食材、同一個單位，所以「誰產最多品鮮蘑菇」是有意義的問題，答案也可能是一隻全能型。
...
**`clearBoxFilter()` 只清篩選，刻意不動排序。** 以前它連排序一起清回「加入順序」，註解寫的理由是「否則新增的那隻會跑到中間去」—— 但那是把兩件事混在一起：**篩選會讓新增的那隻完全不出現**（預設的皮卡丘常常不符合條件），而**排序只是換位置**，換位置早就由 `addBtn` 的 `scrollIntoView(data-i)` 解決了。實際踩過（2026-09-08）：用「圖鑑編號」在看箱子，按一下「新增一隻」整個列表跳回加入順序，看起來像排序自己壞掉。`addBtn` / 截圖存入 / JSON 匯入三條路徑都走這個函式，所以三條都中。第 11d 節有斷言。

代價是**測試不能再靠 `clearBoxFilter()` 把排序歸零** —— 要歸零就自己設 `fltSort`（第 11e 節開頭就是這樣做）。

篩選列還有：**專長**與**食材**兩個篩選（食材那個見下方「個體產能」）、**排序**（加入順序／圖鑑編號／產能／選定食材的產量／等級高→低／專長／主技能 —— 定義在 `BOX_SORTS`，同鍵時一律 `|| a-b` 回到加入順序）、**展開／收起全部**（只作用在目前篩選看得到的那些 —— 一次攤開 60 隻要建一萬多個 `<option>`）、**重複偵測**。
...
npm test                         # smoke：引擎、單調性、窮舉不變量、雙後端、Sheet 往返（含離線沖出、失敗重試、「資料存在哪裡」的文案）、Worker、截圖匯入、箱子篩選／排序／重複偵測／暱稱與 escape、JSON 追加／取代、文案與資源版本一致、schema 偏移
