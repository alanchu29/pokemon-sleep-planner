...
### 食材利用率（`ingUtilNotice`）：低利用率的原因**不是鍋容量**

以前 UI 只在食材**不足**（`mp.idleMeals > 0`）時警告，但實際更常見的是相反：食材大量過剩而畫面一片安靜 —— 使用者只會覺得「推演怎麼都不選食材型」。低於 `ING_UTIL_WARN`（60%）就出聲，推演與自組隊伍共用同一份。

**但這一節真正的重點是診斷不能歸錯原因。** 第一版的文案寫「一週最多 21 餐 × 鍋容量，所以食材型再多也吃不下，要先提高鍋子容量」—— 那是**錯的**，而且會害使用者去加一個沒用的東西。實測（使用者當場抓到）：

| 隊伍 | 鍋容量上限 | 實際煮掉 | 21 餐 |
|---|---|---|---|
| 全食材隊 potEff 81 | 21×81 = **1701** | 1373（28%） | **全滿** |
| 樹果隊把鍋子加到 332 | 6972 | 992（**52%**） | 全滿 |

鍋子加到 4 倍，利用率一動也不動。真正的原因是**木桶效應**：`cooks = min(floor(pool[i]/a))`，每道料理要湊齊它需要的**每一味**，最缺的那一味決定能煮幾次。所以產量高但種類不均的話，多的那幾味只能堆著（那支全食材隊剩 1259 個蜂蜜，卻因為可可／蛋只有 104 個而煮不了高價食譜）。

所以文案分成**兩件不同的事**，而且判斷方式不同：

1. **「已經煮的 21 餐用不掉你的食材」** —— 看利用率與 `leftover`，要列出**剩最多的那幾味**（那才是能拿去做決定的東西）。
2. **「還有更貴的食譜連進鍋的機會都沒有」** —— 看 `POOL` 前段有沒有 `cnt > potEff` 的。**只有這一項加鍋容量才有用**，而且它不會讓上面堆著的食材變少。

早一版把兩者混在一起，輸出會同時說「不是鍋子太小」又說「加鍋子才有用」—— 自相矛盾。第 11j 節直接斷言那兩句不能同時出現。
